	...
	<view style="position: relative;">
		...
		<view class="box" ref="box" id="box">
			...
			<view class="kind width center">
				<view class="kind-item" ref="kindItem" v-for="item in kindList" :key="item.id" @click="changeActive(item.id)">
					{{item.name}}
				</view>
			</view>
...
<script>
	...
	export default {
		...
		mounted() {
			var item = this.$refs.kindItem;
			item[0].$el.classList.add('active')
			this.getArticleList()
		},
		methods: {
			showNav() {
			...
			changeActive(id) {
				let arr = this.$refs.kindItem;
				for (var i = 0; i < arr.length; i++) {
					arr[i].$el.classList.remove('active')
				}
				arr[id].$el.classList.add('active')
				this.selected = id
			},
